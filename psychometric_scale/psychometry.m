...
for iList = 1:numel(list)
    ...
    switch test_name
        case {'NORRIS','CUSTOM_EVA'} % EVA - response format
            ...
            data.psychometry.(test_name).result = askEVA(aga,questionOption,displayOption, scaleOption, key);
        otherwise % multiple choice - response format
            aga.caseN=listImage.psychometry.case;
            aga.caseV=listImage.psychometry.caseV;
            data.psychometry.(test_name).result = askPsychometricScale(aga , displayOption, key );
    end
    KbReleaseWait;
end
